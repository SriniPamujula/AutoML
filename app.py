...
def read_plotly_html(path: str) -> str:
    """Read a Plotly HTML file and return its content as a string."""
...
def run_pipeline(df: pd.DataFrame, filename: str, target_column: str, base_output_dir: str) -> str:
    """Run the full ML pipeline and return the rendered results template.

    Steps: EDA → Visualizations → Preprocessing → Training → Results

    Args:
        df: Input DataFrame.
        filename: Display name of the file.
        target_column: Name of the target column.
        base_output_dir: Base directory for generated chart output.

    Returns:
        Rendered HTML string for the results template.
    """
    ...
    # Create a unique output directory per request to avoid concurrent overwrites
    ...
    if not df_numeric.empty:
        boxplot_path = generate_boxplot(df_numeric, os.path.join(output_dir, 'boxplot.html'))

    heatmap_path = ''
    if df_numeric.shape[1] >= 2:
        heatmap_path = generate_heatmap(df_numeric, target_column, os.path.join(output_dir, 'heatmap.html'))
    ...
    comparison_html = comparison.comparison_table.to_html(
        classes='table table-sm table-dark-custom', index=False, float_format='%.4f'
    )
...
def create_app(config_class=Config):
    ...
    def run_sample(sample_id):
        ...
        try:
            ...
            return run_pipeline(
                ...
                base_output_dir=app.config['GENERATED_IMAGES_FOLDER'],
            )
    ...
    def analyze():
        """Run full ML pipeline on user-uploaded data."""
